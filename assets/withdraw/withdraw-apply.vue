...
import {tj } from '@/api/common/index'
...
const submitForm = () => {
  ...
  if(userInfo.value.detail.auditStatusAdvanced != '1'){
    _toast('Please complete real-name authentication')
    return
  }
  if (route.query?.icon != 'card') {
    if (address.value == '') {
