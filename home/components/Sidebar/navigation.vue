<template>
  <div class="list">
    ...
    <div class="item" @click="changeStyle">
      <van-icon name="setting-o" size="20px" class="itemImg" color="#333" v-if="isDarkMode == 'dark'" />
      <van-icon name="setting-o" size="20px" class="itemImg" v-if="isDarkMode == 'light'" />
      <div class="itemRight">
        <span>{{ _t18(`changeStyle`) }}</span>
        <svg-load name="jiantou-y" class="rightImg"></svg-load>
      </div>
    </div>
  </div>
...
const isDarkMode = ref(localStorage.getItem(storageDict.THEME) || 'dark')
...
const changeStyle = () => {
  ...
  window.__theme = isDarkMode.value
  document.body.setAttribute('theme', isDarkMode.value)
  localStorage.setItem(storageDict.THEME, isDarkMode.value)
}
