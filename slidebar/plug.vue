      ...
      <div v-if="!['das'].includes(_getConfig('_APP_ENV'))">
        <div class="teamInfo" >
          ...
          <div>
            ...
            <span class="ff-num" v-if="teamInfo.l1list">{{ teamInfo.l1list.length || 0 }}</span>
          </div>
          <!-- 二代人数 -->
          <div>
            <p>{{ _t18('plug_twoNum', ['aams']) }}</p>
            <span class="ff-num" v-if="teamInfo.l2list">{{ teamInfo.l2list.length  || 0 }}</span>
          </div>
          <!-- 三代人数 -->
          <div>
            <p>{{ _t18('plug_threeNum', ['aams']) }}</p>
            <span class="ff-num" v-if="teamInfo.l3list">{{ teamInfo.l3list.length  || 0 }}</span>
          </div>
          <!-- 推广总人数 -->
          <div>
            <p>{{ _t18('plug_sum', ['aams']) }}</p>
            <span class="fw-num">{{ teamInfo.countLAll || 0 }}</span>
          </div>
          <!-- 佣金金额 -->
          <div>
            <p>{{ _t18('plug_amount', ['aams']) }}</p>
            <span class="ff-num">{{ teamInfo.allAmount || 0 }}USDT</span>
          </div>
          <div></div>
        </div>
      </div> 
...
<script setup>
import { pageInfo } from '@/api/common/index'
