<script setup>
import { lineuplist } from '@/api/common/index'
import { useRouter, useRoute } from 'vue-router'
const $router = useRouter()
const list = ref('')
const getPageInfo = async () => {
  const res = await lineuplist()
  console.log(res)
  list.value = res.data
}
getPageInfo()
...
const goJoin = async (info) => {
  $router.push('/mutualDetail?lineUpId=' + info.lineUpId)
}
</script>
<template>
  <div class="mutualBox">
    ...
    <div class="contentBox">
      ...
      <p class="title">{{$t('mutualInfo.m2')}}</p>
      <div class="list" v-for="item in list">
        ...
        <div class="button" @click="goJoin(item)">{{$t('mutualInfo.m5')}}</div>
      </div>
    </div>
  </div>
</template>
